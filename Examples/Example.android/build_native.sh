...
if [[ "$buildexternalsfromsource" ]]; then
    ...
    "$NDK_ROOT"/ndk-build -C "$APP_ANDROID_ROOT" $* \
        "NDK_MODULE_PATH=${COCOS2DX_ROOT}:${COCOS2DX_ROOT}/cocos2dx/platform/third_party/android/source" NDK_DEBUG=1
else
    echo "Using prebuilt externals"
    set -x
    "$NDK_ROOT"/ndk-build -C "$APP_ANDROID_ROOT" $* \
        "NDK_MODULE_PATH=${COCOS2DX_ROOT}:${COCOS2DX_ROOT}/cocos2dx/platform/third_party/android/prebuilt" NDK_DEBUG=1
fi

chmod 777 "$APP_ANDROID_ROOT" -R
