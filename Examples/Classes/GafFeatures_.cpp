...
GafFeatures::MenuItemPair_t GafFeatures::addBtn(const char * text, float px, float py, SEL_MenuHandler handler, float k)
{
...
bool GafFeatures::init()
{
    ...
    items->addObject(addBtn("Play/Pause"    , 0.95f, 0.95f,            menu_selector(GafFeatures::playpause), s).first);
    items->addObject(addBtn("Restart"       , 0.95f, 0.95f - dp,       menu_selector(GafFeatures::restart), s).first);
    items->addObject(addBtn("B"             , 0.75f, 0.95f - dp * 2.f, menu_selector(GafFeatures::black), s).first);
    items->addObject(addBtn("W"             , 0.85f, 0.95f - dp * 2.f, menu_selector(GafFeatures::white), s).first);
    items->addObject(addBtn("G"             , 0.95f, 0.95f - dp * 2.f, menu_selector(GafFeatures::gray), s).first);
    //items->addObject(addBtn("Remove"        , 0.95f, 0.95f - dp * 3.5f, menu_selector(GafFeatures::removeOne), s).first);
    //items->addObject(addBtn("Add"           , 0.95f, 0.95f - dp * 3.f, menu_selector(GafFeatures::addOne), s).first);
    //items->addObject(addBtn("1"             , 0.75f, 0.95f - dp * 4.f, menu_selector(GafFeatures::set1), s).first);
    //items->addObject(addBtn("5"             , 0.85f, 0.95f - dp * 4.f, menu_selector(GafFeatures::set5), s).first);
    //items->addObject(addBtn("10"            , 0.95f, 0.95f - dp * 4.f, menu_selector(GafFeatures::set10), s).first);
    items->addObject(addBtn("Frame-"        , 0.95f, 0.95f - dp * 4.0f, menu_selector(GafFeatures::prevFrame), s).first);
    items->addObject(addBtn("Frame+"        , 0.95f, 0.95f - dp * 4.5f, menu_selector(GafFeatures::nextFrame), s).first);
    //items->addObject(addBtn("Full cleanup"  , 0.95f, 0.95f - dp * 5.0f, menu_selector(GafFeatures::cleanup), s).first);
    items->addObject(addBtn("Anim-"         , 0.95f, 0.95f - dp * 5.5f, menu_selector(GafFeatures::prev_anim), s).first);
    items->addObject(addBtn("Anim+"         , 0.95f, 0.95f - dp * 6.0f, menu_selector(GafFeatures::next_anim), s).first);

    m_nextSeq = addBtn("Next sequence"      , 0.95f, 0.95f - dp * 2.5f, menu_selector(GafFeatures::nextSequence), s);
    m_prevSeq = addBtn("Prev sequence"      , 0.95f, 0.95f - dp * 3.f, menu_selector(GafFeatures::prevSequence), s);
