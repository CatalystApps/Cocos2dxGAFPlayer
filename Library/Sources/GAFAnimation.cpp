#include "GAFPrecompiled.h"
#include "GAFAnimation.h"
#include "GAFAnimationSequence.h"
#include "GAFAsset.h"
#include "GAFAnimatedObject.h"
#include "GAFTimeline.h"

GAFAnimation::GAFAnimation()
:
m_asset(nullptr),
m_timeline(nullptr),
m_currentFrameIndex(GAF_FIRST_FRAME_INDEX),
_sequenceDelegate(nullptr),
m_animationPlaybackDelegate(nullptr),
m_totalFrameCount(0),
m_isRunning(false),
m_isReversed(false),
m_isLooped(true)
{

}

GAFAnimation::~GAFAnimation()
{
    CC_SAFE_RELEASE(m_asset);
}

void GAFAnimation::setSequenceDelegate(GAFSequenceDelegate * delegate)
{
    _sequenceDelegate = delegate;
}


bool GAFAnimation::init(GAFAsset * anAssetData, GAFTimeline* tl)
{
    CCAssert(anAssetData, "anAssetData data should not be nil");
    CCAssert(tl, "Timeline data should not be nil");
    
    if (m_asset != anAssetData)
    {
        CC_SAFE_RELEASE(m_asset);
        m_asset = anAssetData;
        CC_SAFE_RETAIN(m_asset);
    }
    
    if (m_timeline != tl)
    {
        CC_SAFE_RELEASE(m_timeline);
        m_timeline = tl;
        CC_SAFE_RETAIN(m_timeline);
    }
    
    m_currentSequenceStart = m_currentFrameIndex = GAF_FIRST_FRAME_INDEX;

	m_currentSequenceEnd = m_totalFrameCount = m_timeline->getFramesCount();
    return true;
}

void GAFAnimation::clearSequence()
{
    m_currentSequenceStart = GAF_FIRST_FRAME_INDEX;
    m_currentSequenceEnd = m_totalFrameCount;
}

void GAFAnimation::step()
{
    if (! m_isReversed)
    {
        if (m_currentFrameIndex < m_currentSequenceStart)
        {
            m_currentFrameIndex = m_currentSequenceStart;
        }

        if (_sequenceDelegate && m_timeline)
        {
            const GAFAnimationSequence * seq = m_timeline->getSequenceByLastFrame(m_currentFrameIndex);
            if (seq)
            {
                _sequenceDelegate->onFinishSequence(dynamic_cast<GAFAnimatedObject*>(this), seq->name);
            }
        }

        if (m_currentFrameIndex >= m_currentSequenceEnd)
        {
            if (m_isLooped)
            {
                m_currentFrameIndex = m_currentSequenceStart;
                
                if (m_animationPlaybackDelegate)
                {
                    m_animationPlaybackDelegate->onAnimationStartedNextLoopDelegate(this);
                }
            }
            else
            {
                setAnimationRunning(false);

                if (m_animationPlaybackDelegate)
                {
                    m_animationPlaybackDelegate->onAnimationFinishedPlayDelegate(this);
                }

                return;
            }
        }

        processAnimation();

		if (isAnimationRunning())
		{
			++m_currentFrameIndex;
		}
    }
    else
    {
        // If switched to reverse after final frame played
        if (m_currentFrameIndex >= m_currentSequenceEnd)
        {
            m_currentFrameIndex = m_currentSequenceEnd - 1;
        }

        if (_sequenceDelegate && m_timeline)
        {
            const GAFAnimationSequence * seq = m_timeline->getSequenceByFirstFrame(m_currentFrameIndex + 1);
            if (seq)
            {
                _sequenceDelegate->onFinishSequence(dynamic_cast<GAFAnimatedObject*>(this), seq->name);
            }
        }

        if (m_currentFrameIndex < m_currentSequenceStart)
        {
            if (m_isLooped)
            {
                m_currentFrameIndex = m_currentSequenceEnd - 1;

                if (m_animationPlaybackDelegate)
                {
                    m_animationPlaybackDelegate->onAnimationStartedNextLoopDelegate(this);
                }
            }
            else
            {
                setAnimationRunning(false);

                if (m_animationPlaybackDelegate)
                {
                    m_animationPlaybackDelegate->onAnimationFinishedPlayDelegate(this);
                }

                return;
            }
        }

        processAnimation();

		if (isAnimationRunning())
		{
			--m_currentFrameIndex;
		}
    }
}

void GAFAnimation::processAnimation()
{

}

bool GAFAnimation::isDone() const
{
    if (m_isLooped)
    {
        return false;
    }
    else
    {
        if (!m_isReversed)
        {
            return m_currentFrameIndex > m_totalFrameCount;
        }
        else
        {
            return m_currentFrameIndex < GAF_FIRST_FRAME_INDEX - 1;
        }
    }
}

uint32_t GAFAnimation::totalFrameCount() const
{
    return m_totalFrameCount;
}

uint32_t GAFAnimation::currentFrameIndex() const
{
    return m_currentFrameIndex;
}

bool GAFAnimation::isAnimationRunning() const
{
    return m_isRunning;
}

bool GAFAnimation::isLooped() const
{
    return m_isLooped;
}

void GAFAnimation::setLooped(bool looped)
{
    m_isLooped = looped;
}

void GAFAnimation::start()
{
    if (!m_isRunning)
    {
        m_currentFrameIndex = GAF_FIRST_FRAME_INDEX;
        setAnimationRunning(true);
    }
}

void GAFAnimation::pauseAnimation()
{
    if (m_isRunning)
    {
        setAnimationRunning(false);
    }
}

void GAFAnimation::resumeAnimation()
{
    if (!m_isRunning)
    {
        setAnimationRunning(true);
    }
}

void GAFAnimation::stop()
{
    if (m_isRunning)
    {
        m_currentFrameIndex = GAF_FIRST_FRAME_INDEX;
        setAnimationRunning(false);
    }
}

bool GAFAnimation::gotoAndStop(const std::string& frameLabel)
{
    int f = getStartFrame(frameLabel);
    if (-1 == f)
    {
        return false;
    }
    return gotoAndStop(f);
}

bool GAFAnimation::gotoAndStop(uint32_t frameNumber)
{
    if (setFrame(frameNumber))
    {
        setAnimationRunning(false);
        return true;
    }
    return false;
}

bool GAFAnimation::gotoAndPlay(const std::string& frameLabel)
{
    int f = getStartFrame(frameLabel);
    if (-1 == f)
    {
        return false;
    }
    return gotoAndPlay(f);
}

bool GAFAnimation::gotoAndPlay(uint32_t frameNumber)
{
    if (setFrame(frameNumber))
    {
        setAnimationRunning(true);
        return true;
    }
    return false;
}


bool GAFAnimation::setFrame(uint32_t index)
{
    if (index < m_totalFrameCount)
    {
        m_currentFrameIndex = index;
        processAnimation();
        return true;
    }
    return false;
}

int GAFAnimation::getStartFrame(const std::string& sequence_name)
{
    if (!m_asset)
    {
        return -1;
    }
    const GAFAnimationSequence * seq = m_timeline->getSequence(sequence_name);
    if (seq)
    {
        return seq->startFrameNo;
    }
    return -1;
}

int GAFAnimation::getEndFrame(const std::string& sequence_name)
{
    if (!m_asset)
    {
        return -1;
    }
    const GAFAnimationSequence * seq = m_timeline->getSequence(sequence_name);
    if (seq)
    {
        return seq->endFrameNo;
    }
    return -1;
}

bool GAFAnimation::playSequence(const std::string& name, bool looped, bool _resume, AnimSetSequenceHint hint)
{
    if (!m_asset || !m_timeline)
    {
        return false;
    }
    
    if (name.empty())
    {
        return false;
    }
    
    int s = getStartFrame(name);
    int e = getEndFrame(name);
    if (-1 == s || -1 == e)
    {
        return false;
    }
    
    m_currentSequenceStart = s;
    m_currentSequenceEnd = e;

    if (m_currentFrameIndex < m_currentSequenceStart || m_currentFrameIndex >m_currentSequenceEnd)
    {
        m_currentFrameIndex = m_currentSequenceStart;
    }
    else
    {
        if (hint == ASSH_RESTART)
        {
            m_currentFrameIndex = m_currentSequenceStart;
        }
        else
        {
            // new hints may appear
        }
    }
    setLooped(looped);
    if (_resume)
    {
        resumeAnimation();
    }
    else
    {
        stop();
    }
    return true;
}
bool GAFAnimation::hasSequences() const
{
    return !m_timeline->getAnimationSequences().empty();
}
bool GAFAnimation::isReversed() const
{
    return m_isReversed;
}
void GAFAnimation::setReversed(bool reversed)
{
    m_isReversed = reversed;
}

void GAFAnimation::setAnimationRunning(bool value)
{
    m_isRunning = value;
}

bool GAFAnimation::getIsRunning() const
{
    return m_isRunning;
}

void GAFAnimation::setAnimationPlaybackDelegate(GAFAnimationPlaybackDelegate* delegate)
{
    m_animationPlaybackDelegate = delegate;
}
